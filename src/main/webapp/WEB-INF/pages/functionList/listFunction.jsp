...
<script type="text/javascript">
   ...
   function functionDel(functionId,deviceId)
   {
       if(confirm('您确定删除该功能吗？'))
       {
    	   var hiddenPageIndex = document.getElementById('hiddenPageIndex').value; 
           window.location.href="<%=path%>/admin/removeInterface.do?interfaceId="+functionId+"&deviceId="+deviceId+"&pageIndex="+1+"&pageSize="+10+"&hiddenPageIndex="+hiddenPageIndex;
       }
   }
...
<body>
		...
		<tbody>
			<c:forEach var="function" items="${data}" varStatus="status">
				<tr>
					...
					<td><img id="img1" src="images/test/image1.jpg" width="30px"
						height="30px" data-action="zoom"></td>
					<td><input type="button" value="删除" class="crud_device"
						onclick="functionDel(${function.interfaceId},${device.deviceId })" /> <input
						type="button" value="查看" class="crud_device"
						onclick="functionCheck(${function.interfaceId})" /> <input
						type="button" value="修改" class="crud_device"
						onclick="editDeviceFunction(${function.interfaceId})" /></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	<div id="page" style="text-align: center; margin-top: 5px;">
		...
		<c:if test="${pageIndex>1}">
			<td><a href="<%=path%>/listDevice.do?pageIndex=1&pageSize=10">首页</a></td>
			<td><a
	...
	<input type="button" value="返回" onclick="back(${hiddenPageIndex})"
		style="background: #F7B52C; height: 30; margin-left: 5px; margin-top: 10px; font-weight: bolder; font-size: 15px; border: 3px solid #0090DB; height: 30px; width: 100; position: absolute; background: #0090DB; color: #FFEDF1;"
		onMouseOver="this.style.backgroundColor='#0074A6';"
		onMouseOut="this.style.backgroundColor ='#0090DB';" />
	<input type="button" value="新增功能"
		onclick="addFunction(${device.deviceId})"
		style="background: #F7B52C; height: 30; margin-left: 105px; margin-top: 10px; font-weight: bolder; font-size: 15px; border: 3px solid #0090DB; height: 30px; width: 100; position: absolute; background: #0090DB; color: #FFEDF1;"
		onMouseOver="this.style.backgroundColor='#0074A6';"
		onMouseOut="this.style.backgroundColor ='#0090DB';" />
